...
pub fn compile_function<'ctx>(
    ...
    if let Some(function_value) = ctx.lookup_function_value(function_idx) {
        return function_value;
    }

    // Fetch the IR for this function
    let function = ctx.get_function(function_idx);

    // Create the underlying LLVM value for this function.
    let function_value = ctx.create_function_value(function_idx);
